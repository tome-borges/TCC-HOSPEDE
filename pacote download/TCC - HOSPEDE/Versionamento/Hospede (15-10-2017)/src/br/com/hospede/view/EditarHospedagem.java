...
public class EditarHospedagem extends InterfaceFactory {
    ...
    public DTOReserva getHospedagem() {
        ...
        try {
            ...
            String hora, data,novaData;
            hora = formatarHora.format(saidaAntiga);
            data = formatarData.format(saida.getDate());
            novaData = data+" "+hora;
            
            
            Date novaDataSaida = new Date();
            novaDataSaida = formatarDataGeral.parse(novaData);
            reserva.setEntrada(entrada.getDate());
            reserva.setSaida(novaDataSaida);
        } catch (Exception erro) {
        }
