...
public class EntrandoOuSaindoDAO {
    ...
    public ArrayList<DTOReserva> exibirClienteEntrandoHoje() {
        ArrayList<DTOReserva> lista = new ArrayList<DTOReserva>();
        ...
        try {
            ...
            statement = banco.setStatement("SELECT \n"
                    ...
                    + "WHERE r.tipo = 'RESERVAR' AND r.status = 'ABERTA' AND to_char(r.entrada,'YYYY-MM-dd') = '"+formatarDataPesquisa.format(dataAtual)+"'");
            resultSet = statement.executeQuery();

            while (resultSet.next()) {
                DTOReserva entrandoOuSaindo = new DTOReserva();
                entrandoOuSaindo.setNome_cliente(resultSet.getString(1));
                entrandoOuSaindo.setNumero_quarto(resultSet.getString(2));
                entrandoOuSaindo.setEntrada(resultSet.getTimestamp(3));
                entrandoOuSaindo.setSaida(resultSet.getTimestamp(4));
                entrandoOuSaindo.setCelular_cliente(resultSet.getString(5));
    ...
    public ArrayList<DTOReserva> exibirClienteSaindoHoje() {
        ArrayList<DTOReserva> lista = new ArrayList<DTOReserva>();
        banco = ConectarBanco.getInstance();
        banco.conectar();
        try {
            ...
            statement = banco.setStatement("SELECT \n"
                    ...
                    + "LEFT JOIN public.reserva r ON q.id = r.id_quarto\n"
                    + "LEFT JOIN public.cliente c ON r.id_cliente = c.id\n"
                    + "WHERE r.tipo = 'HOSPEDAR' AND r.status = 'ABERTA' AND to_char(r.saida,'YYYY-MM-dd') = '"+formatarDataPesquisa.format(dataAtual)+"'");
            resultSet = statement.executeQuery();
