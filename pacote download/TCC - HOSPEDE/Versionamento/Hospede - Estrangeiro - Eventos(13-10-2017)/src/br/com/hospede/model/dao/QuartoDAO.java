    ...
    public DTOQuarto consultarQuarto(int numero) {
        ...
        try {
            statement = banco.setStatement("SELECT\n"
                    ...
                    + "        FROM\n"
                    + "        public.quarto q \n"
                    + "        LEFT JOIN (SELECT * FROM reserva WHERE (entrada = '" + data + "' AND status='ABERTA') OR\n"
                    + "                                               (TIPO = 'HOSPEDAR' AND '" + data + "' BETWEEN entrada AND saida AND status='ABERTA'))r ON q.id = r.id_quarto\n"
    ...
    public boolean checarDisponibilidadeData(DTOReserva reserva, DTOQuarto quarto) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                reservaBanco.setEntrada(resultSet.getTimestamp(6));
                reservaBanco.setSaida(resultSet.getTimestamp(7));

                reservas.add(reservaBanco);
            }

        } catch (Exception erro) {
            JOptionPane.showMessageDialog(null, "Erro ao editar checar disponibilidade:\n" + erro.getMessage());
        }
