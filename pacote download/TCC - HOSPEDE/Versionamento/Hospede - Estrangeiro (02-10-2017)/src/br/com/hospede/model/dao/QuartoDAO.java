...
public class QuartoDAO {
    ...
    public void atualizarQuarto() {
        ...
        banco.conectar();
        DTOQuarto quarto;
        ArrayList<DTOQuarto> quartos = new ArrayList<DTOQuarto>();

        //Primeiro pego todos os quartos.
        try {
            statement = banco.setStatement("SELECT * FROM quarto ");
            resultSet = statement.executeQuery();
        ...
        try {
            ...
            for (int posicao = 0; posicao < quartos.size(); posicao++) {
                statement = banco.setStatement("SELECT * FROM RESERVA WHERE ID_QUARTO ='" + quartos.get(posicao).getId_quarto() + "'  AND TIPO= 'RESERVAR' AND STATUS = 'ABERTA'");
                resultSet = statement.executeQuery();
        ...
        //Já tenho os quartos e as reservas, agora edito situacao com base na data de entrada de cada reserva.
        int posicaoReserva;
        for (int posicaoQuarto = 0; posicaoQuarto < quartos.size(); posicaoQuarto++) {

            //For para todas as reservas.
            for (posicaoReserva = 0; posicaoReserva < reservas.size(); posicaoReserva++) {

                
                try{
                if (reservas.get(posicaoReserva).getId_quarto() == quartos.get(posicaoQuarto).getId_quarto() &&
                    formatarData.format(reservas.get(posicaoReserva).getEntrada()).equals(formatarData.format(dataAtual))) {
                    statement = banco.setStatement("UPDATE quarto SET situacao = 'RESERVADO' WHERE id = '" + quartos.get(posicaoQuarto).getId_quarto() + "'");
                    statement.execute();
                } else {
                    statement = banco.setStatement("UPDATE quarto SET situacao = 'LIVRE' WHERE id = '" + quartos.get(posicaoQuarto).getId_quarto() + "'");
                    statement.execute();
                }
                }catch(Exception erro){
