...
public class PassageiroDAO {
    ...
    public void cadastrarPassageiro(DTOPassageiro passageiro){
        ...
        try{
            statement = banco.setStatement("INSERT INTO passageiros (id_passeio, id_hospedagem_responsavel, nome_passageiro,"
                    + " quarto_hospedagem, cpf_passageiro, pagamento)"
                    + "VALUES (?,?,?,?,?,?)");
            ...
            statement.setString(6, "NÃO PAGO");
        ...
        ArrayList<DTOPassageiro> listaPasseio = new ArrayList<DTOPassageiro>();
          ...
          try{
            ...
            while(resultSet.next()){
                ...
                passeioInterno.setId(resultSet.getInt(11));
    ...
    public boolean editarPassageiro(DTOPassageiro passageiro) {
        banco = ConectarBanco.getInstance();
        banco.conectar();
        boolean editou = false;

        try {
            statement = banco.setStatement("UPDATE passageiros SET pagamento=?"
                    + "WHERE id = ?");
            statement.setString(1, "PAGO");
            statement.setInt(2, passageiro.getId());

            statement.execute();
            editou = true;

        } catch (Exception erro) {
            JOptionPane.showMessageDialog(null, "Erro ao editar passageiro:\n" + erro.getMessage());
        }
