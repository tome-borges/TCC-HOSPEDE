...
public class PasseioDAO {
    ...
    public DTOPasseio consultarPasseio(String nome) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                passeio.setData(resultSet.getTimestamp(4));
    ...
    public boolean editarPasseio(DTOPasseio passeio) {
        ...
        try {
            statement = banco.setStatement("UPDATE passeio SET ingresso=?, data=?, descricao=?, status=?, titulo = ? WHERE id = ? and status = 'ABERTO'");
            ...
            statement.setInt(6, passeio.getId());
    ...
    public ArrayList<DTOPassageiro> listarPasseiosFecharConta(DTOReserva reserva) {
        ...
        try {
            statement = banco.setStatement("SELECT\n"
                    ...
                    + "\n"
                    + "WHERE T3.ID = ? AND T1.PAGAMENTO = 'NÃO PAGO' AND T2.STATUS <> 'CANCELADO'\n"
                    + "\n"
                    + "ORDER BY T2.TITULO");
            ...
            while (resultSet.next()) {
                ...
                passeio.setId(resultSet.getInt(10));
    ...
    public ArrayList<DTOPassageiro> listarPasseiosReceita(int id) {
        DTOPassageiro passeio;
        ArrayList<DTOPassageiro> listaPasseio = new ArrayList<DTOPassageiro>();
        banco = ConectarBanco.getInstance();
        banco.conectar();
        try {
            statement = banco.setStatement("SELECT\n"
                    ...
                    + "INNER JOIN CLIENTE T4 ON T3.ID_CLIENTE = T4.ID\n"
                    + "\n"
                    + "WHERE T3.ID = ? AND T2.STATUS <> 'CANCELADO'\n"
                    + "\n"
                    + "ORDER BY T2.TITULO");
            ...
            JOptionPane.showMessageDialog(null, "Erro ao listar passeios para receita:\n" + erro.getMessage());
        }
        banco.desconectar();
        return listaPasseio;
    }
