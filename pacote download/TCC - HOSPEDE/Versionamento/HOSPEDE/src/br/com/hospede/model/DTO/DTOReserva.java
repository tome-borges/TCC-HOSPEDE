...
public class DTOReserva {
    ...
    public DTOReserva() {
                    ...
                    URL resourceDeletar = getClass().getResource("/br/com/hospede/view/imagens/Close.png");
    ...
    public void setSaldo(Double saldo) {
     ...
     public void totalPagar(DTOQuarto quarto){
        ...
        DateTime entrada;
        try {
            entrada = new DateTime(formatarData.parse(formatarData.format(getEntrada())));
            DateTime saida = new DateTime(formatarData.parse(formatarData.format(getSaida())));
            dias_hospedados = Days.daysBetween(entrada, saida).getDays();
        } catch (ParseException ex) {
            Logger.getLogger(DTOReserva.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        
        //Multiplica os dias hospedados com o preço da diário do quarto, passado por parâmetro.

        totalPagar = quarto.getDiaria() * dias_hospedados;
    }
     ...
     public void totalPagarFechar(DTOQuarto quarto){
        ...
        try {
            entrada = new DateTime(formatarData.parse(formatarData.format(getEntrada())));
            DateTime dataAtualFechar = new DateTime(formatarData.parse(formatarData.format(dataAtual)));
            
            if(formatarData.format(getEntrada()).equals(formatarData.format(dataAtual)) ){
                dias_hospedados = 1;
            } else {
                dias_hospedados = Days.daysBetween(entrada, dataAtualFechar).getDays();
            }
        } catch (ParseException ex) {
            Logger.getLogger(DTOReserva.class.getName()).log(Level.SEVERE, null, ex);
        }
