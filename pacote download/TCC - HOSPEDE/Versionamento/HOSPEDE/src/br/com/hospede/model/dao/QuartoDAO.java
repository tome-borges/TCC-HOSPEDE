...
public class QuartoDAO {
    ...
    public boolean cadastrarQuarto(DTOQuarto quarto) {
        ...
        try {
            ...
            statement.setDouble(4, quarto.getDiaria());
    ...
    public DTOQuarto consultarQuarto(int numero) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                Quarto.setDiaria(resultSet.getDouble(5));
    ...
    public ArrayList<DTOQuarto> listarQuartoSelecionar(Date entrada, Date saida) {
        ...
        try {
            statement = banco.setStatement("SELECT\n"
                    ...
                    + "                    LEFT JOIN (SELECT * FROM reserva WHERE status='ABERTA')r ON q.id = r.id_quarto\n"
                    + "                    LEFT JOIN cliente c on r.id_cliente = c.id\n"
                    + "                    WHERE '"+formatarDataHora.format(entrada)+"' between to_char(r.entrada,'YYYY-MM-DD HH24:MI') and to_char(r.saida,'YYYY-MM-DD HH24:MI') OR\n"
                    + "                           '"+formatarDataHora.format(saida)+"' between to_char(r.entrada,'YYYY-MM-DD HH24:MI') and to_char(r.saida,'YYYY-MM-DD HH24:MI')\n"
                    + "                    ORDER BY numero) \n"
                    + "                    ORDER BY numero");
    ...
    public DTOQuarto getQuartoPorID(DTOReserva reserva) {
        ...
        try {
            ...
            while (resultSet.next()) {
                quarto = new DTOQuarto(0, "", "", "", null, "", "", null, null);
