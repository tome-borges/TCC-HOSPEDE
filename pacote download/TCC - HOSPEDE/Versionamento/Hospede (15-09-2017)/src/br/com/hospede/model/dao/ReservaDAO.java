...
public class ReservaDAO {
    ...
    //----------------------------------------------------------------------------------------------------------------------------
    public ArrayList<DTOReserva> consultarReservaNome(DTOCliente clientePesquisar) {
        banco = ConectarBanco.getInstance();
        banco.conectar();
        ArrayList<DTOReserva> listaReserva = new ArrayList<DTOReserva>();
        

        try {
            statement = banco.setStatement("SELECT * FROM reserva WHERE nome like ? AND tipo = 'RESERVAR'");
