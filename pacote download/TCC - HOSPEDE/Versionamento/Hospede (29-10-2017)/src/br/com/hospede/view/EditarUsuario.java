...
public class EditarUsuario extends InterfaceFactory {
    ...
    public static EditarUsuario instance = null;
    ...
    //Construtor..
    public EditarUsuario(JButton botoes[], JMenuItem itensDeMenu[], JTextField campoDeTexto[], DTOUsuario usuarioLogado, Object[] modelosTabelas) {
        ...
        painelPessoal.add(btnSalvar);

        botoes[7] = btnSalvar;
        campoDeTexto[1] = txtLogin;

        getRootPane().setDefaultButton(btnSalvar); //Deixa o botão Editar em foco para o KeyListener.
        eventos = new ControlarEventos( botoes,itensDeMenu,campoDeTexto, usuarioLogado, modelosTabelas);
        btnSalvar.addActionListener(eventos);
    }

    public DTOUsuario getUsuario() {
      DTOUsuario usuario = new DTOUsuario(id_usuario,0,txtLogin.getText(),pswSenha.getText(),txtEmail.getText()
                                      ,txtTelefone.getText(),funcoes[funcao.getSelectedIndex()]);
        return usuario;

    }
        
    public void recebeUsuario(DTOUsuario usuario) {
    ...
    public static EditarUsuario getInstance( JButton botoes[],JMenuItem itensDeMenu[], JTextField campoDeTexto[], DTOUsuario usuarioLogado, Object[] modelosTabelas) {
        if (instance == null) {
            return instance = new EditarUsuario( botoes, itensDeMenu, campoDeTexto, usuarioLogado, modelosTabelas);
        }
        return instance;
    }
}
