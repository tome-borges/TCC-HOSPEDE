    ...
    public ArrayList<DTOQuarto> listarQuartoSelecionar(Date entrada, Date saida) {
        ...
        try {
            statement = banco.setStatement("SELECT\n"
                    ...
                    + "LEFT JOIN (SELECT * FROM reserva WHERE status='ABERTA')r ON q.id = r.id_quarto\n"
                    + "LEFT JOIN cliente c on r.id_cliente = c.id\n"
                    + "WHERE STATUS <> 'ORGANIZANDO' AND to_date('"+formatarDataHora.format(entrada)+"','YYYY-MM-dd HH24:mi')> to_date(to_char(r.saida,'YYYY-MM-dd HH:mm'),'YYYY-MM-dd HH24:mi') OR\n"

                           + "STATUS <> 'ORGANIZANDO' AND  to_date('"+formatarDataHora.format(saida)+"','YYYY-MM-dd HH24:mi')< to_date(to_char(r.entrada,'YYYY-MM-dd HH:mm'),'YYYY-MM-dd HH24:mi') \n"
