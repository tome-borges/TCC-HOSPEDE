...
public class EditarReserva extends InterfaceFactory {

    public static EditarReserva instance;
    ...
    private final JLabel lblNome, lblCPF, lblRG, lblEstado, lblCidade, lblEndereco,
            ...
            lblEntrada, lblSaida, lblDiaria, lblPagamento, lblHora;
    ...
    private int id_cliente, id_reserva, id_quarto;
    ...
    SpinnerDateModel modeloSpinner;
    JSpinner.DateEditor editar ;
    private JSpinner horas;
    private SimpleDateFormat            formatarDataGeral = new SimpleDateFormat("dd/MM/yyyy HH:mm");
    private SimpleDateFormat            formatarData = new SimpleDateFormat("dd/MM/yyyy");
    private SimpleDateFormat            formatarHora = new SimpleDateFormat("HH:mm");

    public EditarReserva(JButton botoes[], JMenuItem itensDeMenu[], JTextField camposDeTexto[], DTOUsuario usuarioLogado, Object[] modelosTabelas) {
        ...
        modeloSpinner = new SpinnerDateModel(new Date(), null, null, Calendar.LONG);
        horas = new JSpinner(modeloSpinner);
        editar = new JSpinner.DateEditor(horas,"HH:mm");
        horas.setEditor(editar);
        horas.setSize(130, 30);
        horas.setLocation(280, 365);
        
        lblHora = new JLabel("Hora:");
        lblHora.setSize(130, 30);
        lblHora.setLocation(240, 365);
        ...
        getRootPane().setDefaultButton(btnSalvar); //Deixa o botão Reservar em foco para o KeyListener.
    ...
    public DTOReserva getReserva() {
        ...
        try {
            ...
            String data, hora, dataHora;
            //Entrada.
            data = formatarData.format(entrada.getDate());
            hora = formatarHora.format(modeloSpinner.getDate());
            dataHora = data +" "+ hora;
            dataEntrada = formatarDataGeral.parse(dataHora);
            
            //Saida
            data = formatarData.format(saida.getDate());
            hora = formatarHora.format(modeloSpinner.getDate());
            dataHora = data +" "+ hora;
            dataSaida = formatarDataGeral.parse(dataHora);
            
            reserva.setEntrada(dataEntrada);
            reserva.setSaida(dataSaida);
        } catch (Exception erro) {
    ...
    public void receberReserva(DTOReserva reserva)  {
        ...
        try{
        modeloSpinner.setValue(formatarHora.parse(formatarHora.format(reserva.getEntrada())));
        }catch(Exception erro){
    ...
    public static EditarReserva getInstance(JButton botoes[], JMenuItem itensDeMenu[], JTextField camposDeTexto[], DTOUsuario usuarioLogado, Object[] modelosTabelas) {
        if (instance == null) {
            instance = new EditarReserva(botoes, itensDeMenu, camposDeTexto, usuarioLogado,modelosTabelas);
        }
        return instance;
    }
}
